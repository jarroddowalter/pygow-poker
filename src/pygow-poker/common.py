...
def deal_game():  ## returns list of participant hands
    ...
    game_hands = []
    participants = 7  ## max of 7 - 6 player seats and dealer

    ##shuffle
    random.shuffle(GAME_DECK)
    shuffled_deck = GAME_DECK

    ##deal
    i = 0
    while i < participants:
        game_hands.append(list(shuffled_deck[:7]))
        del shuffled_deck[:7]
        participants -= 1
...
def get_outcome(hand):  ## returns outcome as a dict from hand
    ...
    for h in possible_five_card_hands:
        ...
        if len(result) == 5:
            ...
            if result[0] == "JKR" and result[-1][:2] != "14":
                outcome["straights"].append(
                    [result[1], result[2], result[3], result[4], "JKR"]
                )
    ...
    if (
        len(hand) == 7
        and outcome["seven_card_straight_flush"]
        and not outcome["has_joker"]
    ):
        outcome["rank"] = "7 Card Straight Flush, No Joker"
        if outcome["seven_card_straight_flush"][0][:2] == "14":
            outcome["rank_points"] = 30001
        else:
            outcome["rank_points"] = 30000 + int(outcome["seven_card_straight_flush"][0][:2])
        return outcome

    ## Royal Flush Plus Royal Match			20000 + 1-2 royal match rank (A-K > K-Q of same suite/no joker)
    if (
        len(hand) == 7
        and outcome["straight_flushes"]
        and (
            outcome["straight_flushes"][-1][0][:2] == "10"
            or (
                outcome["straight_flushes"][-1][0] == "JKR"
                and outcome["straight_flushes"][-1][1][:2] == "11"
            )
        )
    ):
        ...
        # 	return outcome
        if (
            r_match[0][-1] == r_match[1][-1]
            and r_match[0][:2] == "12"
            and r_match[1][:2] == "13"
        ):  ## check Q K suit match
            outcome["rank"] = "Royal Flush Plus Royal Match"
    ...
    if len(hand) == 7 and outcome["seven_card_straight_flush"] and outcome["has_joker"]:
        ...
        if outcome["seven_card_straight_flush"][0][:2] == "14":
            ...
            outcome["seven_card_straight_flush"][0] == "JKR"
            and outcome["seven_card_straight_flush"][1][:2] == "09"
        ):
            outcome["rank_points"] = 10000 + 8
        else:
            outcome["rank_points"] = 10000 + int(
                outcome["seven_card_straight_flush"][0][:2]
            )
        return outcome

    ## Five Aces							9000
    if "14" in outcome["multiples"] and len(outcome["multiples"]["14"]) == 5:
        outcome["rank"] = "Five Aces"
        outcome["rank_points"] = 9000
        return outcome

    ## Royal Flush							8000
    if outcome["straight_flushes"] and (
        outcome["straight_flushes"][-1][0][:2] == "10"
        or (
            outcome["straight_flushes"][-1][0] == "JKR"
            and outcome["straight_flushes"][-1][1][:2] == "11"
    ...
    if outcome["straight_flushes"]:
        ...
        if (
            outcome["straight_flushes"][0][0][:2] == "14"
            or outcome["straight_flushes"][0][0] == "JKR"
        ):
    ...
    if outcome["straights"]:
        ...
        if outcome["straights"][-1][0][:2] == "14" or (
            outcome["straights"][-1][-5] == "JKR"
            and outcome["straights"][-1][-4][:2] == "02"
        ):
            outcome["rank_points"] = 3000 + 1
        elif (
            outcome["straights"][-1][-5] == "JKR"
            and outcome["straights"][-1][-1][:2] == "14"
        ):
    ...
    if (
        len(outcome["multiples_keys"]) == 1
        and len(outcome["multiples"][outcome["multiples_keys"][0]]) == 3
    ):
    ...
    if (
        len(outcome["multiples_keys"]) >= 2
        and len(outcome["multiples"][outcome["multiples_keys"][-1]]) == 2
        and len(outcome["multiples"][outcome["multiples_keys"][-2]]) == 2
    ):
        ...
        outcome["rank_points"] = (
            1000
            + int(outcome["multiples_keys"][-1])
            + int(outcome["multiples_keys"][-2]) / 100
            + high_card_points
        )
        return outcome
    ...
    if (
        len(outcome["multiples_keys"]) == 1
        and len(outcome["multiples"][outcome["multiples_keys"][0]]) == 2
    ):
        outcome["rank"] = "Pair"
        ...
        if len(outcome["high_card_order"]) >= 3:
            if outcome["high_card_order"][-1] == "JKR":
                ...
                high_card_three_points = (
                    int(outcome["high_card_order"][-3][:2]) / 1000000
                )
            else:
        ...
        outcome["rank_points"] = (
            100
            + int(outcome["multiples_keys"][-1])
            + high_card_one_points
            + high_card_two_points
            + high_card_three_points
        )
        return outcome
    ...
    if outcome["high_card_order"]:
        if (
            outcome["high_card_order"][-1] == "JKR"
            or outcome["high_card_order"][-1][:2] == "14"
        ):
            outcome["rank"] = "A High"
        ...
        if outcome["high_card_order"][-1] == "JKR":
            outcome["rank_points"] = (
                ...
                + high_card_two_points
                + high_card_three_points
                + high_card_four_points
            )
        else:
            outcome["rank_points"] = (
                int(outcome["high_card_order"][-1][:2])
                + high_card_one_points
                + high_card_two_points
                + high_card_three_points
                + high_card_four_points
...
):  ## takes a high hand list and a low hand list and returns a split dict
    split = {
        "high": {},
        "low": {},
    }
