...
def determine_winner(player_split: dict, dealer_split: dict):  ## returns result
    if dealer_split["high"]["rank"] == "A High":  ## Push on A High Pai Gow
        ...
        player_split["high"]["rank_points"] > dealer_split["high"]["rank_points"]
        and player_split["low"]["rank_points"] > dealer_split["low"]["rank_points"]
    ):
        return "Player Wins"
    elif (
        player_split["high"]["rank_points"] > dealer_split["high"]["rank_points"]
        or player_split["low"]["rank_points"] > dealer_split["low"]["rank_points"]
    ):
        return "Push"
    else:
        return "Dealer Wins"
